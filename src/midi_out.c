...
void midi_out_global_shutdown(void) {
    if (g_port)   { MIDIPortDispose(g_port);   g_port   = 0; }
...
const char *midi_out_port_name(int idx) {
    ...
    if (!ep) { snprintf(buf, sizeof(buf), "Device %d", idx); return buf; }
...
MidiOut *midi_out_open(int idx) {
    MIDIEndpointRef ep = MIDIGetDestination((ItemCount)idx);
    if (!ep || !g_client || !g_port) return NULL;
...
#define MAX_PORTS 64
typedef struct { int client; int port; char name[256]; } AlsaPort;
...
static void refresh_ports(void) {
    ...
    while (snd_seq_query_next_client(g_seq, ci) >= 0 && g_nports < MAX_PORTS) {
        ...
        if (c == 0 || c == snd_seq_client_id(g_seq)) continue;
...
int  midi_out_port_count(void) { refresh_ports(); return g_nports; }
const char *midi_out_port_name(int idx) {
    refresh_ports();
...
MidiOut *midi_out_open(int idx) {
    refresh_ports();
    if (!g_seq || idx < 0 || idx >= g_nports) return NULL;
    MidiOut *m = calloc(1, sizeof(*m));
...
void midi_out_close(MidiOut *m) {
    if (!m) return;
    if (g_seq) snd_seq_disconnect_to(g_seq, g_myport, m->dest_client, m->dest_port);
    free(m);
}

void midi_out_send(MidiOut *m, const uint8_t *msg, int len) {
    if (!m || !g_seq || len < 1) return;
    snd_seq_event_t ev;
    snd_seq_ev_clear(&ev);
