...
static uint16_t rb_u16(RBuf* r) {
  ...
  uint16_t v = r->p[0] | ((uint16_t)r->p[1] << 8);
  r->p += 2;
  return v;
}
...
static void rb_raw(RBuf* r, void* dst, int n) {
  ...
  if (av < n) {
    memcpy(dst, r->p, av < 0 ? 0 : av);
    memset((uint8_t*)dst + (av < 0 ? 0 : av), 0, n - (av < 0 ? 0 : av));
    r->p = r->end;
    return;
  }
  memcpy(dst, r->p, n);
...
static void rb_strn(RBuf* r, char* dst, int n) {
  rb_raw(r, dst, n);
  dst[n - 1] = '\0';
}
...
static bool pattern_has_data(const Pattern* p) {
  for (int t = 0; t < PATTERN_TRACKS; t++)
    for (int si = 0; si < p->len; si++) {
      const PatternStep* s = &p->steps[t][si];
      if (s->note != NOTE_EMPTY)
        return true;
      for (int fi = 0; fi < FX_PER_STEP; fi++)
        if (s->fx[fi] != TRACKER_EMPTY)
          return true;
    }
  return false;
}
...
bool tracker_save(const TrackerSong* song, const char* path) {
  ...
  // META
  {
    ...
    wb_u8(&b, (uint8_t)(song->bpm & 0xFF));
    wb_u8(&b, song->swing);
    wb_u8(&b, song->scale_root);
    wb_u8(&b, song->scale_idx);
...
bool tracker_load(TrackerSong* song, const char* path) {
  ...
  for (int si = 0; si < num_sects; si++) {
    ...
    if (strcmp(tag, "META") == 0) {
      ...
      uint8_t bpm_lo = rb_u8(&c);
      song->swing = rb_u8(&c);
      song->scale_root = rb_u8(&c);
      song->scale_idx = rb_u8(&c);
      song->loop = rb_u8(&c) & 1;
      ...
      for (int i = 0; i < cnt; i++) {
        ...
        for (int t = 0; t < ntracks; t++)
          for (int s = 0; s < plen; s++) {
            uint8_t note = rb_u8(&c), vel = rb_u8(&c), inst = rb_u8(&c);
            uint8_t fx0 = rb_u8(&c), fx1 = rb_u8(&c), fxv0 = rb_u8(&c), fxv1 = rb_u8(&c);
            if (t >= PATTERN_TRACKS)
              continue;  // skip tracks beyond capacity
            PatternStep* st = &p->steps[t][s];
            st->note = note;
            st->velocity = vel;
            st->instrument = inst;
            st->fx[0] = fx0;
...
bool tracker_save_pattern(const Pattern* pat, const char* path) {
  WBuf b = {0};
  wb_raw(&b, "RPTP", 4);
  wb_u16(&b, 2);  // version 2: multi-track
  wb_u16(&b, pat->len);
  wb_u8(&b, PATTERN_TRACKS);
  for (int t = 0; t < PATTERN_TRACKS; t++)
    for (int i = 0; i < (int)pat->len; i++) {
      const PatternStep* s = &pat->steps[t][i];
      wb_u8(&b, s->note);
      wb_u8(&b, s->velocity);
      wb_u8(&b, s->instrument);
...
bool tracker_load_pattern(Pattern* pat, const char* path) {
  ...
  if (len == 0)
    len = 1;
  uint8_t ntracks = rb_u8(&r);
  pattern_reset(pat, len);
  for (int t = 0; t < ntracks; t++)
    for (int i = 0; i < len; i++) {
      uint8_t note = rb_u8(&r), vel = rb_u8(&r), inst = rb_u8(&r);
      uint8_t fx[FX_PER_STEP], fxv[FX_PER_STEP];
      for (int f = 0; f < FX_PER_STEP; f++) {
        fx[f] = rb_u8(&r);
        fxv[f] = rb_u8(&r);
      }
