...
void midi_in_global_init(void) {
  if (snd_seq_open(&g_in_seq, "default", SND_SEQ_OPEN_INPUT, 0) < 0) {
    g_in_seq = NULL;
    return;
  }
...
void midi_in_global_init(void) {
  ...
  if (g_in_count > MAX_IN_DEVS)
    g_in_count = MAX_IN_DEVS;
...
void midi_in_global_shutdown(void) {
  for (int i = 0; i < g_in_count; i++) {
    if (g_in_handles[i]) {
      midiInStop(g_in_handles[i]);
      midiInClose(g_in_handles[i]);
    }
  }
}
