...
typedef enum { FB_NONE, FB_OPEN, FB_SAVE } FBMode;
typedef struct { char name[256]; bool is_dir; } Ent;
...
static bool fmatch(const char *name) {
    ...
    while (tok) {
        if (tok[0] == '*' && tok[1] == '.') {
            ...
            if (ext && strcasecmp(ext, tok + 1) == 0) return true;
        }
        tok = strtok(NULL, " ");
    }
    return false;
...
static int ecmp(const void *a, const void *b) {
    const Ent *ea = a, *eb = b;
    if (!strcmp(ea->name, "..")) return -1;
    if (!strcmp(eb->name, "..")) return  1;
    ...
    while ((de = readdir(d)) && g_cnt < MAX_ENT) {
        ...
        if (isdir) {
            ...
            if (ext && strcasecmp(ext, ".clap") == 0) {
                if (!strstr(g_filt, ".clap")) continue;
                isdir = false;
            }
        }
        if (!isdir) {
            if (!fmatch(n)) continue;
        }
...
static void go_up(void) {
    ...
    if (!last) { g_mode = FB_NONE; g_ready = 0; return; }
    if (last == g_dir) {
        if (!strcmp(g_dir, "/")) { g_mode = FB_NONE; g_ready = 0; return; }
        g_dir[1] = '\0';
    } else {
        *last = '\0';
    }
...
void file_browser_open(const char *title, const char *filter) {
    (void)title;
    g_mode = FB_OPEN; g_ready = 0; g_fname[0] = '\0'; g_fname_ed = false;
    if (!g_dir[0]) strncpy(g_dir, GetWorkingDirectory(), MAX_PATH - 1);
...
void file_browser_save_as(const char *title, const char *def_name) {
    (void)title;
    g_mode = FB_SAVE; g_ready = 0; g_fname_ed = false;
    if (!g_dir[0]) strncpy(g_dir, GetWorkingDirectory(), MAX_PATH - 1);
...
void file_browser_download(const char *p, const char *n) { (void)p; (void)n; }

void file_browser_tick(void) {
    if (g_mode == FB_NONE) return;
    int vis = vis_rows();

    // SELECT+B always cancels
    if (input_held(BTN_SELECT) && input_pressed(BTN_B)) {
        g_mode = FB_NONE; g_ready = 0; return;
    }
    ...
    if (g_fname_ed) {
        ...
        while ((ch = GetCharPressed()) > 0) {
            ...
            if (ch >= 32 && l < sizeof(g_fname) - 2) {
                g_fname[l] = (char)ch; g_fname[l + 1] = '\0';
            }
        }
        ...
        if (input_pressed(BTN_A) || IsKeyPressed(KEY_ENTER)) {
            if (g_fname[0]) {
                ...
                g_ready = 1; g_mode = FB_NONE; g_fname_ed = false;
            }
        }
        if (input_pressed(BTN_B)) g_fname_ed = false;
        return;
    }

    if (fb_rep(BTN_UP) && g_cur > 0) {
        g_cur--;
        if (g_cur < g_scr) g_scr = g_cur;
    }
    if (fb_rep(BTN_DOWN) && g_cur < g_cnt - 1) {
        g_cur++;
        if (g_cur >= g_scr + vis) g_scr = g_cur - vis + 1;
    }
    ...
    if (input_pressed(BTN_B)) go_up();

    if (g_mode == FB_SAVE) {
        if (input_pressed(BTN_Y))     g_fname_ed = true;
...
void file_browser_draw(void) {
    ...
    for (int i = 0; i < vis && (g_scr + i) < g_cnt; i++) {
        ...
        Ent *e = &g_ents[idx];
        char label[260];
        if (!strcmp(e->name, ".."))  snprintf(label, sizeof(label), "[ .. ]");
        else if (e->is_dir)          snprintf(label, sizeof(label), "[%s]", e->name);
