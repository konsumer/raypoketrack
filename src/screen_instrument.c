...
void screen_instrument_update(UIState *ui) {
  ...
  if (ui->midi_in_picker_active) {
    ...
    if (input_pressed(BTN_B)) ui->midi_in_picker_active = false;
    return;
  }
  ...
  // ---- Param rows (INST_PARAM_BASE+) ----
    ...
    if (ui->clap_picker_active) {
      ...
      if (input_pressed(BTN_A)) {
        ...
        if (after > before) ui->inst_row = INST_PARAM_BASE + after;
...
void screen_instrument_draw(UIState *ui) {
  ...
  // MIDI-in CHN row
  {
    ...
    if (cur) DrawText("holdA+UP/DN", PANEL_W - 68, y + (CH_H - FONT_S) / 2, FONT_S - 1, C_DIM);
  }
