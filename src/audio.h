...
typedef struct {
  ...
  uint32_t sample_acc;
  uint16_t row_tick;
  // Per (lane, track): the note/instrument currently sounding
  uint8_t active_note[SONG_CHANNELS][PATTERN_TRACKS];
  uint8_t active_inst[SONG_CHANNELS][PATTERN_TRACKS];  // instrument loaded per (lane,track)

  // Per (lane, track) per-slot unit states + which def created them
